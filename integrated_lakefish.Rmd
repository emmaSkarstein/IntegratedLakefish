...
```{r, plotting shared trout field from the four fish model, eval = run_four_fish_model}
# Predict shared field
four_fish_sharedfield_trout <- predict(four_fish_model, mesh = mesh, 
                                 mask = norway.poly, format = 'sp',
                                 spatial = TRUE, fun = 'linear', 
                                 species = "Salmo_trutta",
                                 n.samples = 1000)

saveRDS(four_fish_sharedfield_trout, "results/four_fish_sharedfield_trout.rds")
four_fish_sharedfield_trout <- readRDS("results/four_fish_sharedfield_trout.rds")

# Plot shared spatial field
ggplot() +
  gg(four_fish_sharedfield_trout$predictions) +
  plot_preferences
```

First, we define a function to predict and plot the species-specific shared field for each of the species.

```{r, plotting shared fields from the four fish model, eval = run_four_fish_model}
predict_and_plot_species <- function(model, species, mask, mesh, plot_preferences){
  # Predict shared field
...
for(fish in fishes) {
  plot_list[[fish]] <- predict_and_plot_species(
    four_fish_model,
    species = fish,
    mask = norway.poly,
    mesh = mesh,
