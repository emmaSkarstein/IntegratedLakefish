...
run_two_fish_model <- TRUE
run_four_fish_model <- FALSE
```
...
perch_survey <- four_fish_survey %>% 
  filter(grepl('Perca_fluviatilis', species))
perch_artsobs <- four_fish_artsobs %>% 
  filter(grepl('Perca_fluviatilis', species))
```
...
# Two species model (test)
```{r, eval = run_two_fish_model}
two_fish_setup <- intModel(
  trout_survey,                    # Survey data trout
  trout_artsobs,                   # Citizen science data trout
  perch_survey,                    # Survey data perch
  perch_artsobs,                   # Citizen science data perch
...
two_fish_setup$changeComponents()

# Change perch_survey_field
## Note: changing copy = "trout_survey_spatial" to model = "trout_survey_field" 
## means that perch_survey_field will be estimated separately, it will just be 
## using a spatial field with the same parameters as the trout model. 
## Or so I thought! But this doesn't seem to be working, based on the model output, where any perch spatial field is missing :(
two_fish_setup$changeComponents(~ perch_survey_spatial(main = geometry, 
    model = trout_survey_field))

# Change perch_artsobs_spatial to be a copy of perch_survey_spatial
two_fish_setup$changeComponents(~ perch_artsobs_spatial(main = geometry, copy = "perch_survey_spatial", hyper = list(beta = list(fixed = TRUE))))

# Add bias fields for a
two_fish_setup$addBias(c("trout_artsobs", "perch_artsobs"))

# Change perch bias field to be a copy of trout bias field
two_fish_setup$changeComponents(~ perch_artsobs_biasField(main = geometry, 
    copy = trout_artsobs_biasField, hyper = list(beta = list(fixed = TRUE))))

two_fish_setup$changeComponents()

two_fish_setup$updateFormula(datasetName = "trout_survey")
two_fish_setup$updateFormula(datasetName = "trout_artsobs")
two_fish_setup$updateFormula(datasetName = "perch_survey")
two_fish_setup$updateFormula(datasetName = "perch_artsobs")


two_fish_model <- fitISDM(two_fish_setup, 
                           options =
...
# Remove spatial field that is shared across all species
#four_fish_setup$changeComponents(removeComponent = "four_fish_survey_spatial")
#four_fish_setup$changeComponents(removeComponent = "four_fish_artsobs_spatial")

# Add copies for the species specific fields
# four_fish_setup$changeComponents(
#   ~ Perca_fluviatilis_artsobs_spatial(Perca_fluviatilis_artsobs_spatial, copy = "Perca_fluviatilis_spatial",
#          hyper = list(beta = list(fixed = TRUE))) +
#      Salmo_trutta_artsobs_spatial(Salmo_trutta_artsobs_spatial, copy = "Salmo_trutta_spatial",
#          hyper = list(beta = list(fixed = TRUE))) +
#      Salvelinus_alpinus_artsobs_spatial(Salvelinus_alpinus_artsobs_spatial, copy = "Salvelinus_alpinus_spatial",
#          hyper = list(beta = list(fixed = TRUE))) +
#      Esox_lucius_artsobs_spatial(Esox_lucius_artsobs_spatial, copy = "Esox_lucius_spatial",
#          hyper = list(beta = list(fixed = TRUE))))
