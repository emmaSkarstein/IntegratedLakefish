...
covariateData <- covariateData %>% mutate(log_area = log(area_km2)) %>% 
  ...
  dplyr::mutate_at(vars(-decimalLongitude, -decimalLatitude), scale2, na.rm = TRUE)
...
trout_setup <- intModel(trout_artsobs, 
                        ...
                        responseCounts = "occurenceStatus", # Not sure if needed since we don't have counts?
                        Projection = proj)
...
four_fish_setup <- intModel(
  ...
  Coordinates = c("x", "y"),      # Names of the coordinate variables
  responsePA = "occurrenceStatus",# What is the name of the response column?
