...
```{r, plotting shared trout field from the four fish model, eval = FALSE}
# Predict shared field
...
# Plot shared spatial field
ggplot() +
  gg(four_fish_sharedfield_trout$speciesPredictions[["Salmo_trutta"]]) +
  plot_preferences
```
...
purrr::reduce(plot_list, `|`) | biasplot
#ggsave("figures/shared_fields.pdf", width = 15, height = 5)

#plot_list$Salmo_trutta + plot_list$Perca_fluviatilis + plot_list
```
