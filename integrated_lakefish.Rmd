...
```{r, predicting from the trout model}
trout_predictions <- predict(trout_model, mesh = mesh, mask = norway.poly, 
                       spatial = TRUE,
                       fun = 'linear', n.samples = 1000)
saveRDS(trout_predictions, "results/trout_predictions.rds")

plot(trout_predictions, whattoplot = 'mean', 
