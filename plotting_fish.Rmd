...
p_survey <- ggplot(Data_survey_df %>% filter(occurrenceStatus == "TRUE"), 
...
ggsave("presence_points.pdf", height = 5, width = 8)
```
...
```{r}
#trout_model <- readRDS()
trout_predictions <- readRDS("results/trout_predictions.rds")
```

```{r, plotting the trout model}
p <- plot(trout_predictions, whattoplot = 'mean',
          plot = FALSE)$predictions$mean +
#  geom_polygon(data = norway, aes(long, lat, group = group), 
#               color="grey80", fill = "grey95") + 
  scale_fill_gradient(low = "darkcyan", high = "gold") +
  coord_map() +
